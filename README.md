...
- Sends are queued during the request and flushed on `shutdown` (after `fastcgi_finish_request()` where available, so there is no visitor-facing latency), then posted with `blocking => true`. This is deliberate: most admin write-actions call `wp_redirect()` + `exit` immediately after the event hook, which would tear down a fire-and-forget (`blocking => false`) request before it transmits. Alerts are de-duplicated within a 5-minute window so an active attack cannot flood the channel; the entire payload is built with `wp_json_encode()` so usernames, IPs, and file paths cannot break or inject into the JSON
...
- **Wordfence integration (avoids duplicate detection):** where Wordfence already covers an event, KW Security relays Wordfence's own detection instead of detecting natively. Real-time events come through Wordfence's `wordfence_security_event` action (`adminLogin`→`admin_login`, `adminLoginNewLocation`→`admin_login_new_ip`, `loginLockout`→`login_lockout`, `block`→`login_blocked`); scan findings (malware, file changes, vulnerable/outdated plugins) are relayed from Wordfence's alert emails, routed into the `malware`, `file_changed`, and `plugin_update_critical` categories (generic ones fall through to `wordfence_alert`). These categories are listed in `KW_Security_Alerts::get_wordfence_sourced()` and their native detection short-circuits to prevent double-alerting. Everything is filterable — `kw_slack_wordfence_sourced` (which categories defer to Wordfence), `kw_slack_wordfence_event_map`, `kw_slack_wordfence_email_routes`, `kw_slack_wordfence_email_skip`, and `kw_slack_is_wordfence_alert`. KW-unique events (admin granted/deleted, password change, Application Password, WooCommerce REST key, defense disabled, Wordfence deactivated) stay native
